...
struct mlx5_ifc_nvme_emulation_cap_bits {
	...
	u8	 nvme_emulated_dev_db_cq_map[0x1];
	u8	 nvme_db_cq_map_set_db_val[0x1];
	u8	 nvme_emulated_dev_eq[0x1];
	u8	 dynamic_vf_db_control[0x1];
	...
	u8	 always_armed_device_db[0x1];
	u8	 reserved_at_e5[0x1b];
...
struct mlx5_ifc_virtio_emulation_cap_bits {
	...
	u8	 device_features_bits_mask_2[0x10];
	u8	 reserved_at_1a[0x1];
	u8	 dynamic_vf_db_control[0x1];
	u8	 db_cq_map_set_db_val[0x1];
...
enum {
	...
	MLX5_VIRTIO_DEVICE_MODIFY_DEVICE_PCI_PARAMS = 1 << 11,
	MLX5_VIRTIO_DEVICE_MODIFY_DYN_VF_DB_RESET = 1 << 13,
...
enum mlx5_obj_type {
	...
	MLX5_OBJ_TYPE_TLP_DEVICE_EMULATION = 0x5a
};
...
enum {
	...
	MLX5_NVME_DEVICE_MODIFY_ALL_REGISTERS_DATA = 1 << 4,
	MLX5_NVME_DEVICE_MODIFY_DOORBELL_RANGE = 1 << 5,
	MLX5_NVME_DEVICE_MODIFY_NUM_MSIX = 1 << 8,
	MLX5_NVME_DEVICE_MODIFY_DYN_VF_MSIX_RESET = 1 << 9,
	MLX5_NVME_DEVICE_MODIFY_DYN_VF_DB_RESET = 1 << 13,
};
...
struct mlx5_ifc_virtio_fs_config_bits {
	...
	u8	   notify_buf_size[0x20];
	u8	   reserved_at_160[0x120];
};
...
struct mlx5_ifc_virtio_net_device_emulation_bits {
	...
	u8	   always_armed_device_db[0x1];
	u8	   dynamic_vf_db_control[0x1];
	u8	   dynamic_vf_db_reset[0x1];
	u8	   reserved_at_6d[0x1];
...
struct mlx5_ifc_virtio_blk_device_emulation_bits {
	...
	u8	   num_free_dynamic_vfs_db[0x10];
...
struct mlx5_ifc_tlp_device_emulation_bits {
    u8     modify_field_select[0x40];
    ...
    u8     enabled[0x1];
    u8     reserved_at_61[0x4];
    u8     pci_hotplug_state[0x3];
    u8     reserved_at_68[0x2];

    u8     always_armed_device_db[0x1];
    u8     reserved_at_6b[0x45];
...
struct mlx5_ifc_nvme_device_emulation_bits {
	...
	u8	   dynamic_vf_db_control[0x1];
	u8	   dynamic_vf_db_reset[0x1];
	u8	   reserved_at_6d[0x3];
	u8	   num_free_dynamic_vfs_msix[0x10];

	u8	   counter_set_id[0x20];

	u8	   reserved_at_a0[0x10];
	u8	   doorbell_range[0x10];
...
struct mlx5_ifc_2err_qp_in_bits {
	u8	opcode[0x10];
	u8	uid[0x10];
...
struct mlx5_ifc_query_emulated_resources_info_in_bits {
	u8	opcode[0x10];
	u8	uid[0x10];
...
struct mlx5_ifc_query_emulated_resources_info_out_bits {
	u8	status[0x8];
	u8	reserved_at_8[0x18];
	...
	u8	num_free_dynamic_msix[0x20];

	u8	num_free_dynamic_db[0x20];
...
// ========================================================================
// TLP Query Test Definitions
// ========================================================================

// Command opcodes for TLP device queries
#define MLX5_CMD_OPCODE_QUERY_EMULATED_FUNCTIONS_INFO 0xb03

// PRM Emulation OpMod definitions for TLP_DEVICES support
#define PRM_EMULATION_OPMOD_NVME_PF         0x0
#define PRM_EMULATION_OPMOD_VIRTIO_NET_PF   0x1
#define PRM_EMULATION_OPMOD_VIRTIO_BLK_PF   0x2
#define PRM_EMULATION_OPMOD_GENERIC_PF      0x6
#define PRM_EMULATION_OPMOD_TLP_DEVICES     0x7

// Input structure for QUERY_EMULATED_FUNCTIONS_INFO command
struct mlx5_ifc_query_emulated_functions_info_in_simple_bits {
    uint32_t opcode_uid;         // [31:16] opcode, [15:0] uid
    uint32_t reserved1;          // reserved
    uint32_t op_mod;             // operation modifier
    uint32_t reserved2[2];       // reserved
    uint32_t host_number_valid;  // host parameters
    uint32_t host_number;
    uint32_t pf_vhca_id;
    uint32_t reserved3[2];       // reserved
};

// Output structure for QUERY_EMULATED_FUNCTIONS_INFO command
struct mlx5_ifc_query_emulated_functions_info_out_simple_bits {
    uint8_t  status;
    uint8_t  reserved1[3];
    uint32_t syndrome;
    uint32_t reserved2[2];
    uint32_t num_emulated_functions;
    uint32_t reserved3[3];
    // Note: Additional function info follows but not parsed in simple version
};
...
// ========================================================================
// End TLP Query Test Definitions
// ========================================================================
